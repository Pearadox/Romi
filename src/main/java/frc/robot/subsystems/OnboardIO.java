...
public class OnboardIO extends SubsystemBase {
  private final DigitalInput m_buttonA = new DigitalInput(Constants.BTN_A);
  private final DigitalOutput m_yellowLed = new DigitalOutput(Constants.YEL_LED);

  // DIO 1 will be one of these two things
  private DigitalInput m_buttonB;
  private DigitalOutput m_greenLed;

  // DIO 2 will be one of these two things
 ...
 /**
  ...
  public OnboardIO(ChannelMode dio1, ChannelMode dio2) {
    if (dio1 == ChannelMode.INPUT) {
      m_buttonB = new DigitalInput(Constants.BTN_B);
    } else {
      m_greenLed = new DigitalOutput(Constants.GREEN_LED);
    }

    if (dio2 == ChannelMode.INPUT) {
      m_buttonC = new DigitalInput(Constants.BTN_C);
    } else {
      m_redLed = new DigitalOutput(Constants.RED_LED);
    }
  }
